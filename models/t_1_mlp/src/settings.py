...
CHECKPOINT_NAME = env_str("CHECKPOINT_NAME", f"t_1_mlp_{DATASET_NAME}.ckpt")
...
# t_1_mlp is the one-step-only variant: ROLLOUT_STEPS is pinned to 1, so the
# rollout loop collapses to a single next-step prediction and the doubling
# curriculum is irrelevant (disabled).
ROLLOUT_STEPS = env_int("MODEL_ROLLOUT_STEPS", 1)
ROLLOUT_DECAY_BASE = env_float("MODEL_ROLLOUT_DECAY_BASE", 0.5)
ROLLOUT_CURRICULUM_EPOCHS = env_int("MODEL_ROLLOUT_CURRICULUM_EPOCHS", 0)
...
OPTUNA_STUDY_NAME = env_str("STUDY_NAME", f"t_1_mlp_{DATASET_NAME}_optimization")
