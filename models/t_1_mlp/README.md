# Single-step MLP Surrogate (`t_1_mlp`)

This directory owns model hyperparameter optimization, final training, and
autoregressive rollout testing for the **one-step** MLP surrogate. The model is
a plain feed-forward MLP trained with L1 loss on the log10 abundances: it maps
`[physical parameters at t, log10 abundances at t]` to
`log10 abundances at t + 1`.

It is the `t_1` arm of the one-step-vs-rollout comparison: training is pinned to
a single next-step prediction (`MODEL_ROLLOUT_STEPS=1`, no rollout curriculum).
The only substantive difference from the sibling `models/t_20_mlp` (20-step
autoregressive-rollout training) is that pinned `ROLLOUT_STEPS` default — the
data pipeline, architecture / Optuna search space, and full-rollout evaluation
in `test.py` are identical, so the two `test_results/` are directly comparable.
...
| `RESULTS_ROOT` | Root of experiment results (default `models/t_1_mlp/results`) |
...
```text
models/t_1_mlp/results/{dataset name}/{sampler}/
```
...
```text
models/t_1_mlp/results/grav_collapse/density/
...
`best_params.json` to
`models/t_1_mlp/results/{dataset}/{sampler}/optimization/`. The study journal is a SQLite file in
...
and rollout plots to `models/t_1_mlp/results/{dataset}/{sampler}/test_results/`.

## SLURM

Submit from this directory (`models/t_1_mlp/`); job logs go to `logs/`:
