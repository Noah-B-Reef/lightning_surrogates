...
autoregressive rollout testing for the **20-step autoregressive-rollout** MLP
surrogate. The model is a plain feed-forward MLP trained with L1 loss on the
log10 abundances: it maps `[physical parameters at t, log10 abundances at t]` to
`log10 abundances at t + 1`.

It is the `t_20` arm of the one-step-vs-rollout comparison: each training sample
is a 20-step window and the model is unrolled on its own predictions over the
full horizon (`MODEL_ROLLOUT_STEPS=20`), reached via a doubling curriculum
(1→2→4→8→16→20) with per-step decay weighting. The only substantive difference
from the sibling `models/t_1_mlp` (one-step training) is that pinned
`ROLLOUT_STEPS` default — the data pipeline, architecture / Optuna search space,
and full-rollout evaluation in `test.py` are identical, so the two
`test_results/` are directly comparable.
...
| `RESULTS_ROOT` | Root of experiment results (default `models/t_20_mlp/results`) |
...
```text
models/t_20_mlp/results/{dataset name}/{sampler}/
```
...
```text
models/t_20_mlp/results/grav_collapse/density/
...
`best_params.json` to
`models/t_20_mlp/results/{dataset}/{sampler}/optimization/`. The study journal is a SQLite file in
...
and rollout plots to `models/t_20_mlp/results/{dataset}/{sampler}/test_results/`.

## SLURM

Submit from this directory (`models/t_20_mlp/`); job logs go to `logs/`:
