...
CHECKPOINT_NAME = env_str("CHECKPOINT_NAME", f"t_20_mlp_{DATASET_NAME}.ckpt")
...
# t_20_mlp is the 20-step autoregressive-rollout variant: each training sample
# is a 20-step window and the model is unrolled on its own predictions over the
# full horizon, reached via the doubling curriculum (1->2->4->8->16->20).
ROLLOUT_STEPS = env_int("MODEL_ROLLOUT_STEPS", 20)
...
OPTUNA_STUDY_NAME = env_str("STUDY_NAME", f"t_20_mlp_{DATASET_NAME}_optimization")
