...
RNN_DIR="$(cd "${SLURM_DIR}/.." && pwd)"
REPO_DIR="$(cd "${RNN_DIR}/../.." && pwd)"
...
# RNN owns its own naming so its results never collide with the MLP's.
CHECKPOINT_NAME="rnn_${DATASET_NAME}.ckpt"
STUDY_NAME="rnn_${DATASET_NAME}_optimization"
...
EXPERIMENT_DIR="${RESULTS_ROOT:-${RNN_DIR}/results}/${DATASET_NAME}/${SAMPLING_PROCEDURE}"
...
print_config_summary() {
    ...
    echo "RESULTS_ROOT=${RESULTS_ROOT:-${RNN_DIR}/results}"
    ...
    echo "MODEL_RNN_CELL_TYPE=${MODEL_RNN_CELL_TYPE:-lstm}"
    echo "MODEL_RNN_NUM_LAYERS=${MODEL_RNN_NUM_LAYERS:-2}"
    echo "MODEL_RNN_HIDDEN_DIM=${MODEL_RNN_HIDDEN_DIM:-256}"
