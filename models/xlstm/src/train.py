...
def load_best_config(config_file):
    ...
    if path.suffix == ".json":
        ...
        return {
            "xlstm_num_blocks": int(payload["xlstm_num_blocks"]),
            "xlstm_hidden_dim": int(payload["xlstm_hidden_dim"]),
    ...
    key_mapping = {
        "num_layers": "xlstm_num_blocks",
        "num_blocks": "xlstm_num_blocks",
        "hidden_units": "xlstm_hidden_dim",
        "xlstm_num_blocks": "xlstm_num_blocks",
        "xlstm_hidden_dim": "xlstm_hidden_dim",
...
def default_config():
    return {
        "xlstm_num_blocks": config.XLSTM_NUM_BLOCKS,
        "xlstm_hidden_dim": config.XLSTM_HIDDEN_DIM,
...
def train_final_model(
    ...
    *,
    ...
    log_dir = results_dir / "lightning_logs" / "xlstm_grav_collapse"
    if log_dir.exists():
    ...
    model_config = {
        ...
        "xlstm_num_blocks": int(best_config["xlstm_num_blocks"]),
        "xlstm_hidden_dim": int(best_config["xlstm_hidden_dim"]),
        "xlstm_num_heads": config.XLSTM_NUM_HEADS,
        "xlstm_cell_type": config.XLSTM_CELL_TYPE,
        "xlstm_dropout": config.XLSTM_DROPOUT,
        "learning_rate": learning_rate,
        "loss_function": loss_function,
        "seed": int(seed),
    ...
    print(
        ...
        f"cell={model_config['xlstm_cell_type']}, "
        f"num_blocks={model_config['xlstm_num_blocks']}, "
        f"hidden_dim={model_config['xlstm_hidden_dim']}, "
        f"num_heads={model_config['xlstm_num_heads']}, "
    ...
    callbacks = [
        ...
        ModelCheckpoint(
            ...
            filename="xlstm_best-{epoch:04d}-{val_loss:.5f}",
            save_top_k=1,
            mode="min",
        ),
        metrics,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train the xLSTM on a split dataset directory.")
