...
def suggest_params(trial):
    ...
    return {
        "xlstm_num_blocks": trial.suggest_int(
            "xlstm_num_blocks",
            search["xlstm_num_blocks"]["low"],
            search["xlstm_num_blocks"]["high"],
        ),
        "xlstm_hidden_dim": trial.suggest_int(
            "xlstm_hidden_dim",
            search["xlstm_hidden_dim"]["low"],
            search["xlstm_hidden_dim"]["high"],
            step=search["xlstm_hidden_dim"]["step"],
        ),
...
def objective(trial, args, split_dir):
    ...
    model_config = {
        ...
        "xlstm_num_blocks": params["xlstm_num_blocks"],
        "xlstm_hidden_dim": params["xlstm_hidden_dim"],
...
def save_best_params(best_params, results_dir, best_value):
    ...
    with txt_path.open("w") as f:
        f.write(f"xlstm_num_blocks={best_params['xlstm_num_blocks']}\n")
        f.write(f"xlstm_hidden_dim={best_params['xlstm_hidden_dim']}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Optimize xLSTM hyperparameters for a split dataset directory (sequential Optuna study)."
    )
    parser.add_argument(
        "dataset_path",
        nargs="?",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Defaults to results/{dataset_name}/{sampler}/optimization.",
    )
