"""Typed defaults for the xLSTM pipeline, read from the environment.
...
DEFAULT_RESULTS_ROOT = env_path("RESULTS_ROOT", XLSTM_DIR / "results")
CHECKPOINT_NAME = env_str("CHECKPOINT_NAME", f"xlstm_{DATASET_NAME}.ckpt")
...
# Model defaults. The xLSTM keeps the MLP's [phys, abund] -> abund_{t+1}
# interface but carries a recurrent hidden state across the trajectory through a
# stack of xLSTM blocks (Beck et al. 2024). XLSTM_NUM_BLOCKS and
# XLSTM_HIDDEN_DIM size the stack; XLSTM_NUM_HEADS sets the per-cell head count
# (hidden_dim must be divisible by it); XLSTM_CELL_TYPE selects the block kind.
XLSTM_NUM_BLOCKS = env_int("MODEL_XLSTM_NUM_BLOCKS", 2)
XLSTM_HIDDEN_DIM = env_int("MODEL_XLSTM_HIDDEN_DIM", 256)
XLSTM_NUM_HEADS = env_int("MODEL_XLSTM_NUM_HEADS", 4)
XLSTM_CELL_TYPE = env_str("MODEL_XLSTM_CELL_TYPE", "slstm")  # slstm | mlstm | mixed
# Dropout applied to the residual cell/FFN sublayer outputs within each block.
XLSTM_DROPOUT = env_float("MODEL_XLSTM_DROPOUT", 0.0)
...
OPTUNA_STUDY_NAME = env_str("STUDY_NAME", f"xlstm_{DATASET_NAME}_optimization")
...
# to LOSS_FUNCTION; Optuna's objective is val_mse (a fixed metric). The xLSTM
# stack depth/width replace the MLP's layers/units. The cell type and head
# count are taken from the environment (XLSTM_CELL_TYPE / XLSTM_NUM_HEADS) and
# not tuned, so a study isolates depth/width/LR for one architecture variant.
OPTUNA_SEARCH_SPACE = {
    "xlstm_num_blocks": {"type": "int", "low": 1, "high": 4},
    "xlstm_hidden_dim": {"type": "int", "low": 128, "high": 1024, "step": 128},
