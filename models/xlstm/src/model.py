...
from xlstm_cells import xLSTMStack
...
CELL_TYPES = ("slstm", "mlstm", "mixed")


class XLSTM(pl.LightningModule):
    """xLSTM (extended LSTM) one-step surrogate with a carried hidden state.
    ...
    Loss:   selectable via config["loss_function"] (l1 | mse | smooth_l1),
            always computed on the log10 abundances.

    The recurrent core is a stack of xLSTM blocks (Beck et al. 2024) carrying a
    hidden state threaded across the trajectory, so each prediction is
    conditioned on history rather than only the current state. config
    ["xlstm_cell_type"] selects the block kind: ``slstm`` (scalar memory,
    exponential gating, memory mixing), ``mlstm`` (matrix memory, covariance
    update), or ``mixed`` (alternating). This mirrors the RNN surrogate's
    lstm/gru selector and keeps the identical [phys, abund] -> abund_{t+1}
    interface and training protocol.
    ...
    input from step 2 onward is the model's own prediction, with gradients (and
    the hidden state) flowing through the whole chain. Step j (0-based) is
    weighted by config["rollout_decay_base"]**j. The training horizon follows a
    doubling curriculum (1, 2, 4, ... capped at rollout_steps), advancing every
    ...
    def __init__(self, config):
        ...
        hidden_dim = int(config["xlstm_hidden_dim"])
        num_blocks = int(config["xlstm_num_blocks"])
        num_heads = int(config.get("xlstm_num_heads", 4))
        output_size = int(config["output_size"])
        self.cell_type = str(config.get("xlstm_cell_type", "slstm")).lower()
        dropout = float(config.get("xlstm_dropout", 0.0))
        ...
        if self.cell_type not in CELL_TYPES:
            raise ValueError(
                f"xlstm_cell_type must be one of {CELL_TYPES}, "
                f"got {self.cell_type!r}"
            )
        if hidden_dim % num_heads != 0:
            raise ValueError(
                f"xlstm_hidden_dim ({hidden_dim}) must be divisible by "
                f"xlstm_num_heads ({num_heads})"
            )
    ...
    def step(self, x, hidden):
        ...
        out, hidden = self.core.step(x, hidden)
